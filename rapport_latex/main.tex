...
environnementales (humidité, câblage, fixation). Cette approche est cohérente
avec la tendance recensée dans la littérature IoT de réduire le temps entre
prototype logiciel et déploiement physique.
...
un tunnel HTTPS sécurisé entre un serveur local et internet. Cette approche
est particulièrement adaptée aux petites exploitations ne disposant pas
d'adresse IP fixe ni de configuration réseau avancée.

\begin{figure}[H]
  \centering
  \begin{subfigure}[b]{0.15\textwidth}
    \centering
    \includegraphics[width=1.8cm]{PlatformIO_logo.png}
    \caption{PlatformIO}
  \end{subfigure}
  \hspace{0.8cm}
  \begin{subfigure}[b]{0.15\textwidth}
    \centering
    \includegraphics[width=1.8cm]{vscode_logo.png}
    \caption{VS Code}
  \end{subfigure}
  \hspace{0.8cm}
  \begin{subfigure}[b]{0.22\textwidth}
    \centering
    \includegraphics[width=3.5cm]{nodejs_logo.png}
    \caption{Node.js}
  \end{subfigure}
  \hspace{0.8cm}
  \begin{subfigure}[b]{0.15\textwidth}
    \centering
    \includegraphics[width=2.5cm]{BambuLab_logo.png}
    \caption{Bambu Lab}
  \end{subfigure}
  \hspace{0.8cm}
  \begin{subfigure}[b]{0.15\textwidth}
    \centering
    \includegraphics[width=2.5cm]{ngrok_logo.png}
    \caption{ngrok}
  \end{subfigure}
...
\begin{center}
\small
\code{Node.js :3000} $\longrightarrow$ \code{ngrok tunnel} $\longrightarrow$ \code{https://xxxx.ngrok-free.app} $\longrightarrow$ \code{Navigateur distant}
\end{center}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.8\textwidth]{esp32_cam_pinout.png}
...
\textit{carte\_aqua}, réalisé sous KiCad~7~\cite{kicad}. On y retrouve les six connecteurs
...
\begin{figure}[H]
  ...
  \caption{Schématique électronique de la carte aqua-ai-monitor (KiCad 7)}
  \label{fig:sch_pcb}
\end{figure}
...
trous de fixation (vias bleus) assurent le maintien mécanique du module.
Les pistes d'alimentation (5~V et GND) sont routées en priorité avec une
largeur supérieure aux pistes signal.

\begin{figure}[H]
  \centering
  \includegraphics[width=0.7\textwidth]{routage_pcb.png}
  \caption{Vue de routage PCB — couche F\_Cu (KiCad 7)}
...
visualiseur intégré de KiCad~7. On distingue clairement l'empreinte du
module ESP32-WROOM-32 au centre (U1), les connecteurs JST-XH à gauche
(J2, J5) et à droite (J1, J3, J4), la résistance CMS R1, et le connecteur
d'alimentation Jack-DC (J6) en haut à droite.

\begin{figure}[H]
  \centering
  \includegraphics[width=0.85\textwidth]{3d_pcb.png}
  \caption{Rendu 3D de la carte aqua-ai-monitor (KiCad 7)}
  \label{fig:3d_pcb}
\end{figure}
...
Le firmware est développé avec PlatformIO~\cite{platformio} sous VS Code, ciblant la
plateforme \code{espressif32} avec le framework Arduino~\cite{espressif_esp32}.
L'organisation suit le modèle orienté objet, avec une classe dédiée par capteur ou
sous-système :
...
utile lors des phases de calibration ou de débogage sur le terrain. En cas
d'erreur de lecture du DHT22, un message \code{"DHT22 KO"} s'affiche à la place
du cycle normal, signalant immédiatement un problème matériel.
...
de l'exploitation aquacole. Pour permettre une surveillance à distance depuis
n'importe quel navigateur, sans configuration de routeur ni adresse IP fixe,
l'outil \textbf{ngrok} a été utilisé. Ngrok crée un tunnel sécurisé HTTPS entre
le port local~3000 et un sous-domaine public généré automatiquement.
...
Le lancement se fait en une seule commande dans un terminal séparé, sans modifier
le code du serveur :

\begin{lstlisting}[caption={Lancement du tunnel ngrok vers le serveur Node.js}]
# Exposer le serveur local sur internet
ngrok http 3000

# ngrok fournit une URL publique de type :
# https://xxxx-xx-xx-xxx-xx.ngrok-free.app
\end{lstlisting}

Le dashboard devient alors accessible depuis un smartphone ou un ordinateur
distant via l'URL générée. Cette approche est particulièrement adaptée aux
petites exploitations aquacoles qui ne disposent pas d'une infrastructure réseau
avancée, car elle ne nécessite aucune ouverture de port sur le routeur.
...
basique : \code{ngrok http -{}-auth="user:password" 3000}, ou d'utiliser un
tunnel ngrok authentifié avec un compte enregistré.
\end{warningbox}
...
de l'espace des features. Comme démontré par Zhuang \textit{et al.}~\cite{zhuang2023bsiforest},
cet algorithme présente une complexité linéaire particulièrement adaptée aux contraintes
des systèmes IoT embarqués.
...
On observe le flux vidéo live de l'ESP32-CAM en haut à gauche, le panneau
d'alertes actives (pH hors plage et turbidité hors plage détectés simultanément
par l'IA et les seuils métier), les cinq cartes métriques organisées par section
\textit{Ambiance} et \textit{Eau}, ainsi que le panneau d'intelligence artificielle
affichant un score d'anomalie à 100\,\% avec la raison détaillée. La date
d'entraînement au 01/01/1970 indique que le modèle n'a pas encore été entraîné
sur des données réelles — il opère uniquement sur les seuils métier.
...
La figure~\ref{fig:site_2} présente l'onglet graphiques avec les cinq
courbes Chart.js mises à jour toutes les 2 secondes sur une fenêtre de 60~points.
On constate la stabilité de la température eau (18,6°C) et de l'humidité (64,7\,\%),
la variabilité de la turbidité autour de 398~NTU avec quelques décrochages,
et la faible amplitude des oscillations de température air (20,10--20,20°C)
révélant la précision du DHT22. Le pH reste anormalement bas à 3,36, confirmant
...
\begin{enumerate}
  \item \textbf{Seuils côté client} : vérification JavaScript des valeurs reçues
        contre des seuils fixes (pH 6,0--9,0, NTU 0--200, températures) — alignés sur
        les seuils métier du microservice Python ;
...
Les valeurs de pH mesurées ($\approx$3,5) indiquent que le capteur n'a pas encore été
calibré avec les solutions tampon appropriées. Conformément aux recommandations de
de~Camargo \textit{et al.}~\cite{camargo2023lowcost}, l'offset \code{PH\_OFFSET} dans
...
Les métriques présentées dans le tableau~\ref{tab:perf} ont été relevées lors des
sessions de test du prototype en conditions réelles. La latence ESP32 → dashboard
a été estimée en observant le décalage entre l'horodatage \code{updatedAt} généré
côté serveur et l'affichage dans le navigateur, tenant compte du cycle de polling
de 2~s. La consommation du module a été mesurée en série avec un multimètre
numérique sur la ligne d'alimentation 5~V en mode transmission continue. Le débit
vidéo MJPEG a été évalué visuellement sur le flux \code{GET /video} en comptant
les frames par seconde sur une durée de 10~secondes. Les temps d'entraînement et
d'inférence ont été chronométrés avec \code{time.time()} côté Python Flask.
...
Temps d'entraînement IA (20 pts) & < 0,5~s & \code{time.time()} Python \\
...
\subsection{Réalisation physique du prototype}

Les figures~\ref{fig:pcb_irl} et~\ref{fig:pcb_irl_2} présentent le prototype
réalisé sur veroboard en boîtier plastique blanc. On identifie l'ESP32-WROOM-32
au centre de la carte, l'écran OLED I²C à sa droite, le module pH PH-4502C en
haut à gauche avec son connecteur BNC, et le capteur de turbidité Keyestudio V1.0
en bas. Le câblage suit le code couleur standard (rouge~: VCC, noir~: GND,
jaune/orange~: signal).
...
\begin{itemize}
  ...
  \item Python $\geq$ 3.11 (\url{https://python.org}) — port du service IA : 5001
  \item PlatformIO (\url{https://platformio.org})
  \item ngrok (optionnel, pour l'accès distant) (\url{https://ngrok.com})
\end{itemize}
